...
def computeOTable(geoTable, vTable):
    ...
    for c in vTable:
        ...
        triplet.append(min(vTable[nextCorner(c)], vTable[prevCorner(c)]), max(vTable[nextCorner(c)], vTable[prevCorner(c)]), c)
        triplets.append(triplet)
    print "triplets", triplet
    print "vTable", triplets
    sortedTriplets = sorted(triplet) #[set([1, 3]), set([1, 2]), set([0, 2, 3]) etc
