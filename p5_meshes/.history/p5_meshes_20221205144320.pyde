...
def computeOTable(geoTable, vTable):
    ...
    for c in vTable:
        if c not in triplets:
            ...
            triplets[i] = tempTrip[min(vTable[nextCorner(c)], vTable[prevCorner(c)]), max(vTable[nextCorner(c)], vTable[prevCorner(c)]), c]
    ...
    for i in range(0, len(sorted_triplets) - 1, 1):
        ...
        # opTable[] = vTable[sorted_triplets[i+1][2]]
        # opTable[triplet_indices] = vTable[sorted_triplets[i][2]]
    return opTable 
