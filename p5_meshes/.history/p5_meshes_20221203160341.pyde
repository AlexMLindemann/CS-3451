...
def read_mesh(filename):
    ...
    real_opTable = []
    for i in opTable: 
        opDict = dict.fromkeys(vTable[opTable[i]], i)
        # real_opTable[opDict[cornerA]] = vTable[sortedTriplets[i+1][2]]
        # real_opTable[opDict[cornerB]] = vTable[sortedTriplets[i][2]]
    print "opDict", opDict
    #debugging 
    print_mesh()
...
def computeOTable(geoTable, vTable):
    ...
    tripletDict = {}
    for c in vTable:
        triplet = [min(vTable[nextCorner(c)], vTable[prevCorner(c)]), max(vTable[nextCorner(c)], vTable[prevCorner(c)]), c]
        tripletDict = {min(vTable[nextCorner(c)], vTable[prevCorner(c)]), max(vTable[nextCorner(c)], vTable[prevCorner(c)]), c}
        triplets.append(triplet)
    
    # tripletDict = [dict() for x in range(len(triplet))]
    # print "dict" , tripletDict
    sortedTriplets = sorted(triplets) #[set([1, 3]), set([1, 2]), set([0, 2, 3]) etc
    #iterate from - to len(sortedTriplets), add 2 to iterator each time

    for i in range(0, len(sortedTriplets) - 1, 1):
        cornerA = sortedTriplets[i][2]
        opTable.append(cornerA)
        cornerB = sortedTriplets[i+1][2]
        opTable.append(cornerB)
        # print "buggs", triplet[vTable[cornerA]]
        # opTable[tripletDict[vTable[cornerA]]] = vTable[sortedTriplets[i+1][2]]
        # opTable[tripletDict[vTable[cornerB]]] = vTable[sortedTriplets[i][2]]
    return opTable 
