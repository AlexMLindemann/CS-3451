...
def computeOTable(geoTable, vTable):
    ...
    triplets = []
    for corner in vTable:
        triplet = {min(vTable[c.nextCorner], vTable[c.prevCorner]), max(vTable[c.nextCorner], vTable[c.prevCorner]), c}
    ...
    for i in range(0, len(sortedTriplets), 2):
        ...
        # oppositeTable[cornerA] = cornerB
        # oppositeTable[cornerB] = cornerA

        opTable[sortedTriplets[i][2]] = sortedTriplets[i+1][2]
        opTable[sortedTriplets[i+1][2]] = sortedTriplets[i][2]
    return opTable 
