...
def interpreter(fname):
    ...
    for line in lines:
        ...
        if words[0] == 'sphere':
            ...
            vertices = (PVector(float(words[1]), float(words[2]), float(words[3])))
        elif words[0] == 'end':
...
def shadow(hit, light):
    ...
    s_origin = add(hit.intersection_point, offset)
    s_direction = PVector.normalize(PVector.sub(light.pos, hit.intersection_point))
    s_ray = Ray(s_origin, s_direction)

    curr = rayIntersect(s_ray)
    if curr.s != None:
        if curr.t < PVector.dist(light.pos, hit.intersection_point):
            return 0
    return 1
class triangle():
...
class sphere():
    ...
    def intersect(self, ray):
        ...
        #=======debugging===============
        if debug_flag:
            print "testing intersection with the sphere whose color is ", self.surface_color.diffuse # change these variable names to match the rest of your code!
        ...
        if 0 < discriminant: 
            ...
            return None
        if debug_flag:
            print "The hit with the smallest t value is ", closest_hit
            print "shape", self.getNormal
            # store sphere info
