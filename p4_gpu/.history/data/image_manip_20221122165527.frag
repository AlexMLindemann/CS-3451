...
void main() { 
  ...
  for (float i = (-blur_radius); i <= blur_radius; i++) {
    for (float j = (-blur_radius); j <= blur_radius; j++) {
      // vec4 ij_coord = texture2D(my_mask, vertTexCoord.xy + vec2(x*texelX, y*texelY));
      // blur_col += ij_coord;
      ...
      //vec4 texture_sum = texture2D(my_texture, vertTexCoord.xy + vec2(x, i * texelSize, x, j * texelSize));
      blur_col = blur_col + texture_col1;
      blur_col = blur_col + texture_col2;
      // blur_col += texture_col1;
      // blur_col += texture_col2; 
      count++;
    }
    //blur_col /= (2*blur_radius)*blur_radius;
    blur_col = blur_col * (1.0/count);
  } 
