...
void main() { 
  ...
  for (int i = 0; i < blur_radius; i++) {
    for (int j = 0; j < blur_radius; j++) {
      ...
      vec2 i_coord = vec2(vertTexCoord.x, i * texelSize);
      vec2 j_coord = vec2(vertTexCoord.y, i * texelSize);
      // texture_col1 = vec4 texture2D(my_texture, i_coord);
      // texture_col2 = vec4 texture2D(my_texture, j_coord);
    }
