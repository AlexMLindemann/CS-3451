...
vablur_flagrying vec3 vertLightDir;
...
void main() { 
	...
	float xz = vertTexCoord.x * 6.28 - 3.14;
	float yz = vertTexCoord.y * 6.28 - 3.14;
	...
	for(int i = 0; i <= 20; i++)	{
		...
		float vector_x = sin(xz) * cosh(yz);
		float vector_y = cos(xz) * sinh(yz);
	...
	gl_FragColor = gl_FragColor = vec4(diffuse*diffuse_color.rgb, 1.0);
	// else{
