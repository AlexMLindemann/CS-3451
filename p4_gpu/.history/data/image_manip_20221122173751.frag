...
void main() { 
  ...
  vec2 texelSize = 1.0/textureSize(my_texture, 0).xy;  //how much to move over by 1 pix. !!does operation to both elements!!
  //calc mask intensity
  if (mask_color.r < 0.1) {
    blur_radius = 6;
  ...
  for (float i = (-blur_radius); i <= blur_radius; i++) {
    for (float j = (-blur_radius); j <= blur_radius; j++) {
      vec2 coord = vertTexCoord.xy + texelSize*vec2(i, j);
      vec4 texture_sum = texture2D(my_texture, coord);
      blur_col = blur_col + texture_sum;
      // blur_col += texture_col1;
      // blur_col += texture_col2; 
      count++;
    } 
  } 
  blur_col /= (2*blur_radius+1)*(2*blur_radius+1); 
