...
void main() { 
  ...
  for (int i = (-radius-blur_radius); i < blur_radius; i++) {
    for (int j = (-radius-blur_radius); j < blur_radius; j++) {
      vec4 ij_coord = texture2D(my_texture, vertTexCoord.xy + vec2(x*textureSize2d, y*textureSize2d));
      blur_col += ij_coord;
