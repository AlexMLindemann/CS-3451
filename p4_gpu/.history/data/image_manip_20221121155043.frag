...
void main() { 
  ...
  for (int i = 0; i < blur_radius; i++) {
    for (int j = 0; j < blur_radius; j++) {
      ...
      float i_coord = (vertTexCoord.x + i * texelSize);
      float j_coord = (vertTexCoord.y + i * texelSize);
      vec4 texture_col1 = vec4 texture2D(my_texture, i_coord);
      vec4 texture_col2 = vec4 texture2D(my_texture, j_coord);
    }
