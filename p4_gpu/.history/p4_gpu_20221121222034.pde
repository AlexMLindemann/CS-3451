...
void draw() {
  ...
  for (int i = 0; i < 50; i++) {
    for (int j = 0; j < 50; j++) {
      beginShape();
      texture(sheep_texture);
      vertex(i * size_quad, j*size_quad, 0, i/50, j/50);
      vertex((i+1)*size_quad, j*size_quad, 0, (i+1)/50, j/50);
      vertex((i+1)*size_quad, (j+1)*size_quad, 0, (i+1)/50, (j+1)/50);
      vertex(i*size_quad, (j+1)*size_quad, 0, i/50, (j+1)/50);
      endShape();
    }
  }
  popMatrix();
