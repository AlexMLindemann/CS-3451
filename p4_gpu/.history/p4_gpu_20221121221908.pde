...
void draw() {
  ...
  for (int i = 0; i < 40; i++) {
    for (int j = 0; j < 40; j++) {
      beginShape();
      texture(sheep_texture);
      vertex(i * size_quad, j*size_quad, 0, i/40, j/40);
      vertex((i+1)*size_quad, j*size_quad, 0, (i+1)/40, j/40);
      vertex((i+1)*size_quad, (j+1)*size_quad, 0, (i+1)/40, (j+1)/40);
      vertex(i*size_quad, (j+1)*size_quad, 0, i/40, (j+1)/40);
      endShape();
    }
  }
  popMatrix();
